...
class Agent_DQN(Agent):
    def __init__(self, env, args):
        ...
        if args.test_dqn:
            ...
            self.Q_network.load_state_dict(torch.load('checkpoints/test16.pt', map_location=self.device))
            self.Q_network.eval()
            ###########################
            # YOUR IMPLEMENTATION HERE #
    ...
    def train(self):
        ...
        for episode in range(self.n_episodes):
            ...
            if episode % 50000 == 0:
                if self.test_n:
                    torch.save(self.Q_network.state_dict(), f'checkpoints/test{self.test_n}_single.pt')
